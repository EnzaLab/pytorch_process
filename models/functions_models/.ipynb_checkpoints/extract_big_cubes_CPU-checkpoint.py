...
def main(): 
    ...
    df = pd.read_csv(f'{login}/complex/gdrive/shared/Proj_FORMAL/deep_satellite/Code/CNN/pytorch_process/dataset/from_all_made_2024/all_dataset_step2_bathy_done_brewin_done_zone_done.csv')
    ...
    torch.save(final_tensor, f"tensor_picture_space_{space_buffer}_all.pt")
    torch.save(latitudes_tensor, f"tensor_latitude_space_{space_buffer}_all.pt")
    torch.save(longitudes_tensor, f"tensor_longitude_space_{space_buffer}_all.pt")
    df_filtered.to_csv(f'tensor_df_space_{space_buffer}_all.csv')
